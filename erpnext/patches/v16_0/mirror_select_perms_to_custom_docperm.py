...
def removed_roles(doctype):
	...
	for changes in frappe.get_all(
		...
		try:
			...
			# an unreadable entry is still a record that something was removed, but it
			# names no role, so it cannot be matched -- leave it to the row check
			continue
...
def execute():
	# Permission Log is what makes the third test possible: it is the only thing that keeps a rule
	# the site created and later deleted distinguishable from one that never existed. It arrived
	# after v15, so on a branch without it that test cannot be evaluated at all, and inserting
	# anyway would silently fall back to re-granting rules the site may have removed on purpose.
	# Say so and do nothing, rather than degrade without telling anyone.
