...
def check_pos_profile_access(pos_profile):
	...
	# select-or-read, not a bare check: a bare has_permission() defaults to `read`, and the shipped
	# POS Profile rows give Sales Manager only `select` while it writes POS Opening Entry and POS
	# Closing Entry. Defaulting to read denies the role the page exists for.
